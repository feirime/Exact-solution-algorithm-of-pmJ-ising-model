...
Пары цепочек рассматриваются поочерёдно как показано на рисунке~\ref{fig:scheme}

\begin{figure}[tbp!]
\begin{center}
\includegraphics[width=350px]{images/scheme_with_spins.png}
\caption {Схема перебора плотности состояний алгоритмом при добавлении каждой новой цепочки спинов. Чёрными рёбрами соединены спины, связи между которыми учитываются на каждом из шагов алгоритма.}
\label{fig:scheme}
\end{center}
\end{figure}
